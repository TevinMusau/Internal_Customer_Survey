...
<body class="bg-white">
    <div class="container">
        ...
        <div class="col-12 mb-5">
            <h4 class="text-center fw-bold">Staff Survey Report</h4>
        </div>

        <div class="col-12 mb-5">
            <h4 class="text-center fw-bold">{{ $supervisor->first_name }} {{ $supervisor->last_name }} ({{ $supervisor->initials }})</h4>
        </div>
        ...
        <div class="col-12 mb-5">
            <h5 class="text-center fw-bold"><strong>Final Rating:</strong> {{ $supervisor_final_rating }} / 5.0</h5>
        </div>

        <div class="d-flex mb-5">
            <div class="col-5 text-end me-3">
                <h6>{{ $supervisor->first_name }} {{ $supervisor->last_name }} ({{ $supervisor->initials }}) Signature:</h6>
            </div>
        ...
        @foreach ($supervisor_survey_questions as $chunk)
            <hr>
                        ...
                        <script>
                            new Chart(document.getElementById('ratingChart{{ $chartCounter }}'), {
                                ...
                                data: {
                                    ...
                                    datasets: [{
                                        data: [
                                            {{ $survey_question->supervisor_survey_result->first()->grading_1_count ?? 0 }}, 
                                            {{ $survey_question->supervisor_survey_result->first()->grading_2_count ?? 0 }}, 
                                            {{ $survey_question->supervisor_survey_result->first()->grading_3_count ?? 0 }}, 
                                            {{ $survey_question->supervisor_survey_result->first()->grading_4_count ?? 0 }}, 
                                            {{ $survey_question->supervisor_survey_result->first()->grading_5_count ?? 0 }}
                                        ],
                                        backgroundColor: [
                                            '#dc3545',
                                            '#fd7e14',
